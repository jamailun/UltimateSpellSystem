...
public class TokenStream {
    ...
    /**
     * Test if the next token matches a type.
     * @param allowedTypes allowed types.
     * @return true if the next token has a specific type.
     */
    public boolean peekIs(@NotNull TokenType @NotNull ... allowedTypes) {
        Preconditions.checkState(hasMore(), "No more data.");
        Preconditions.checkArgument(allowedTypes.length > 0, "Must have at least 1 value.");
        TokenType peeked = peek().getType();
        return Set.of(allowedTypes).contains(peeked);
    }
    ...
    public @NotNull Token nextOrThrow(@NotNull TokenType type, @NotNull String error) {
        Token next = next();
        if(next.getType() != type)
            throw new SyntaxException(next, error);
        return next;
    }
    ...
    public void dropOrThrow(@NotNull TokenType expectedType, @NotNull String error) {
        Token next = next();
        if(next.getType() != expectedType)
            throw new SyntaxException(next, error);
    }
