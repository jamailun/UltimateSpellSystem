...
public final class Tokenizer {
    ...
    private final static Set<Character> OPERATOR_BI_START = Set.of('<', '>', '=', '!', '&', '|', '+', '-');
    ...
    private TokenStream tokenize() {
        ...
        while(chars.hasMore()) {
            ...
            if(OPERATORS_MONO.containsKey(current) || OPERATOR_BI_START.contains(current)) {
                if(chars.hasMore()) {
            ...
            if(ALLOWED_WORD_START.test(String.valueOf(current))) {
                ...
                if("true".equals(word) || "false".equals(word)) {
                    tokens.add(Token.fromBoolean(Boolean.parseBoolean(word), position));
                    continue;
                }
